  ...
  <div id="userProfilePage">
    ...
    <div class="basicInfo">
      <a-card title="基础信息" :headStyle="{ 'text-align': 'center' }" :loading="userInfoLoading">
        <template #extra><a @click="openEditInfoModal">修改</a></template>
        <div class="avatar">
          <!-- <a-avatar
    ...
    <div class="exchangeVip">
      ...
      <a-form layout="vertical" :model="userVipForm" @finish="handleSubmit">
        <a-input-group compact style="text-align: center">
          <a-input
            ...
            placeholder="请输入兑换码"
          />
          <a-button type="primary" @click="doExchangeVip">兑 换</a-button>
        </a-input-group>
      </a-form>
    </div>
...
import Icon, { LoadingOutlined, PlusOutlined, AntDesignOutlined } from '@ant-design/icons-vue'
import {
...
const fetchUserInfo = async () => {
  ...
  const res = await getUserInfoByIdUsingGet()
...
// 编辑信息弹出框
const editInfoModal = ref<boolean>(false)
...
const openEditInfoModal = () => {
  ...
  userInfoForm.birthday = dayjs(userInfo.value.birthday)
 ...
 * 提交编辑信息表单
 */
const doEditInfoSubmit = async () => {
  const userId = userInfoForm.id
  if (!userId) {
    return
  }
  const res = await updateUserInfoUsingPost(userInfoForm)
  if (res.data.code === 0 && res.data.data) {
    message.success('修改成功')
    // 跳转到图片详情页
    editInfoModal.value = false
    fetchUserInfo()
  } else {
